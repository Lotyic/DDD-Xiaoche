...
void Init_TIM8_PWM(u16 arr, u16 psc)
{
	...
	TIM_SetCompare1(TIM8, 0); // BL	后左
	TIM_SetCompare2(TIM8, 0); // FL	前左
	TIM_SetCompare3(TIM8, 0); // FR	前右
	TIM_SetCompare4(TIM8, 0); // BR	后右
}
...
void Init_TIM1_PWM(u16 arr, u16 psc) // 19999  71    4999  71
{
	...
	GPIO_PinRemapConfig(GPIO_FullRemap_TIM1, ENABLE);
	//设置该引脚为复用输出功能,输出TIM1 CH1的PWM脉冲波形
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_9 | GPIO_Pin_11 | GPIO_Pin_13; // TIM_CH1
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP;						  //复用推挽输出
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	...
	TIM_OC1PreloadConfig(TIM1, TIM_OCPreload_Enable); // CH1 ----A PE9
