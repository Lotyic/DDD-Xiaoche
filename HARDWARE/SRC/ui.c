...
char zhiling[][20] = 
{
    ...
    "$DGT:1-1,1!",   // 初始化				0
    "$DGT:3-3,1!",   // 推环	    		1
    "$DGT:2-2,1!",   // 开机动作 不调用		 2
    "$DGT:4-4,1!",   // 拿地上环放爪	   	 3
    "$DGT:5-9,1!",   // 拿地上环			 4
    "$DGT:25-29,1!", // 拿增益环  			 5
    "$DGT:17-23,1!", // 放三分  			 6
    "$DGT:10-16,1!", // 放二分			 	 7
    "$DGT:24-24,1!", // 爪子前伸保持重心	  8
    "$DGT:30-30,1!", // 偷三分伸爪			 9
    "$DGT:31-36,1!", // 偷三分				 10
    "$DGT:37-37,1!", // 偷二分放爪			 11
    "$DGT:38-43,1!", // 偷二分				 12
};

//五个一组分别表示格数，方向，延时停车的延时时长ms，指令数组下标，等待机械臂延时s
//五个一组分别表示格数，方向，延时停车的延时时长ms，指令数组下标，等待机械臂延时s				第一组的方向为200时代表双车运行
//方向0为后 9为编码器后
unsigned char yy11[][5] = {
//			 {2,1,0,255,0},{0,5,0,255,2},{0,3,0,255,0},
//			 {1,1,0,255,0},{0,1,25,255,0},{1,0,0,255,0},
			{0,0,0,0,0},
};//初启1右
unsigned char yy12[][5] = {
			{2,1,0,255,0},{0,5,0,255,2},{0,3,0,104,0},{1,1,0,255,0},{0,5,0,255,0},//前走2格，左转打开，前走1，保护
			{2,4,0,255,0},{1,3,0,255,0},{0,4,0,255,0},{0,5,0,255,0},{2,1,0,255,0},//右转2，左转1，右转，打开，前走2格，
			{0,5,0,101,0},{3,4,0,255,0},
			{0,5,0,103,0},{0,1,20,255,0},{0,0,10,255,0},
			{0,0,0,0,0},};//初启2右
unsigned char yy13[][5] = {
			{2,1,0,255,0},{2,3,0,255,0},{0,4,0,255,10},{0,1,131,255,0},{3,0,0,255,0},
			{0,0,0,0,0},};//初启3右		中间撞
unsigned char yy14[][5] = {
			{2,1,0,255,0},{0,5,0,255,4},{0,3,0,104,0},{2,1,0,255,0},{0,5,0,255,2},{0,4,0,255,0},
			{2,1,0,255,0},{0,5,0,101,2},{0,4,0,255,0},{3,1,0,255,0},
			{0,5,0,103,0},{0,1,20,255,0},{0,0,10,255,0},
            {0,0,0,0,0},};//初启4右		
unsigned char yy15[][5] = {
			{2,1,0,104,0},{2,1,0,255,0},{0,5,0,255,2},{1,4,0,255,0},
			{0,5,0,103,0},{0,1,20,255,0},{0,0,10,255,0},
			{0,0,0,0,0},};//初启5右
unsigned char yy16[][5] = {
			{2,1,0,255,0},{0,3,0,255,0},{0,5,0,104,2},{2,1,0,255,0},{0,4,0,255,0},{1,1,0,255,0},{2,1,0,255,0},
			{0,5,0,101,0},{1,0,0,255,0},{1,4,0,255,0},{2,1,0,255,0},{0,5,0,103,0},{0,1,20,255,0},{0,0,8,255,0},
			{0,0,0,0,0},};//初启6右

//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
unsigned char oo11[][5] = {
		{0,200,20,255,0},{0,1,20,255,0},{1,1,0,255,0},{0,4,0,255,0},{0,2,0,3,10},{2,1,0,255,0},{0,2,0,4,27},{0,3,0,255,0},{2,1,0,1,10},
		{0,4,0,255,0},{1,1,0,0,1},{0,1,20,255,1},{0,0,20,255,0},{0,3,0,255,0},{0,0,10,255,1},{0,1,80,255,0},{0,0,13,5,30},{0,1,10,255,1},{1,0,0,255,1},{0,4,0,1,3},//拿增益环
		{2,1,0,255,0},{0,4,0,255,0},{2,1,0,255,0},{0,6,0,255,0},{0,1,70,255,3},{0,9,70,255,0},{0,6,0,255,0},{0,2,0,0,1},//推一分

		{0,0,20,255,0},
		{0,1,130,255,0},{0,2,0,7,45},{2,0,0,255,0},{0,3,0,255,0},{3,1,0,255,0},
		{0,2,0,3,14},{1,1,0,255,0},{0,2,0,4,27},{0,2,0,3,14},{2,1,0,255,0},{0,2,0,4,25},{0,4,0,255,10},
		{1,1,0,255,0},{0,4,0,255,10},{0,1,50,255,0},{0,2,0,6,38},{1,0,0,255,1},{0,4,0,255,0},{3,1,0,255,0},{0,3,0,255,0},{5,1,0,255,0},
		{0,4,0,255,0},
		{0,1,70,9,10},{0,0,13,10,30},{3,0,0,255,0},{0,4,0,255,0},{3,1,0,255,0},{0,1,60,6,38},
                ...
                {0,0,0,0,0},};//对抗初启1中  双车
unsigned char oo12[][5] = {		
		{0,1,27,0,4},{0,2,0,3,10},{1,1,0,255,0},{0,2,0,4,25},{0,2,0,3,10},{2,1,0,255,0},{0,2,0,4,27},
		{0,4,0,255,0},{0,2,0,1,10},{5,1,0,255,1},{0,4,0,255,0},{2,1,0,255,0},
		{0,6,0,255,0},{0,1,70,255,0},{0,9,70,255,0},{0,6,0,255,0},{0,2,0,0,1},
		{0,0,20,255,0},{0,1,145,255,0},{0,2,0,7,45},{2,0,0,255,0},{0,3,0,255,0},{3,1,0,255,0},
		{0,2,0,3,14},{1,1,0,255,0},{0,2,0,4,25},{0,2,0,3,14},{2,1,0,255,0},{0,2,0,4,25},{0,4,0,255,0},
		{1,1,0,255,0},{0,4,0,255,10},{0,1,60,255,0},{0,2,0,6,38},			//此时如果全部成功则为2 2 2

		{0,0,20,255,0},{0,3,0,3,10},{0,0,15,255,0},{2,1,0,4,25},{0,4,0,3,10},{2,1,0,4,25},{0,4,0,255,0},
		{1,1,0,255,0},{0,4,0,255,0},{0,1,70,255,0},{0,2,0,6,30},
			

			

			
        {0,0,0,0,0},};//对抗初启2中	单车完胜
unsigned char oo13[][5] = {
			
	
	
			{0,0,0,0,0},};//对抗初启3中 双车
unsigned char oo14[][5] = {
			
			{0,0,0,0,0},};//对抗单车 平局15分
unsigned char oo15[][5] = {
			 {0,0,0,0,0}}; //对抗，左边出发，前推左近
unsigned char oo16[][5] = {
			{0,0,0,0,0},};//对抗，右边出发，前推右近
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
unsigned char zz11[][5] = {
			 {2,1,0,255,0},{0,5,0,104,2},{2,1,0,255,0},{0,4,0,255,0},{2,1,0,255,0},{0,5,0,255,2},{0,3,0,255,0},
			 {2,1,0,255,0},{0,5,0,101,0},{0,4,0,255,0},{3,1,0,255,0},{0,5,0,103,0},{0,1,20,255,0},{0,0,10,255,0},
			 {0,0,0,0,0},};//初启1左
unsigned char zz12[][5] = {
		       {2,1,0,255,0},{0,5,0,104,0},{5,1,0,255,0},{0,4,0,255,0},
			 {4,1,0,255,0}, {0,4,0,255,0},{1,1,0,255,0}, {0,3,0,255,0},
		       {1,1,0,255,0},
			 {0,5,0,103,0},{0,1,20,255,0},{0,0,10,255,0},
                   {0,0,0,0,0},};//对抗初启2左
unsigned char zz13[][5] = {
	             {2,1,0,255,0},{0,5,0,104,2},{5,1,0,255,0},{0,5,0,101,0},//前走2格，打开，前走5格，保护
			 {1,0,0,255,0},{0,5,0,255,3},{0,4,0,255,0},//后退1格，右转，
			 {5,1,0,255,0},
			 {0,5,0,103,0},{0,1,20,255,0},{0,0,10,255,0},
                   {0,0,0,0,0},};//对抗初启3左
unsigned char zz14[][5] = {
			 {2,1,0,255,0},{0,5,0,255,2},{0,4,0,255,0},
			 {1,1,0,255,0},{0,1,25,255,0},{1,0,0,255,0},
			 {0,5,0,255,2},{0,3,0,255,0},{2,1,0,255,0},
			 {0,5,0,1,4},{0,1,10,255,0},{0,5,0,2,4},
			 {0,0,9,255,0},{0,5,0,255,5},{0,4,0,255,0},{3,1,0,255,0},
			 {0,1,118,255,0},{0,5,0,255,4},{0,6,0,255,0},{0,1,90,255,0},{0,5,0,3,4},
			 {0,0,0,0,0},};//初启4左
unsigned char zz15[][5] = {
			 {2,1,0,255,0},{0,5,0,255,2},{0,4,0,255,0},
			 {1,1,0,255,0},{0,5,0,255,2},{0,3,0,255,0},{2,1,0,255,0},
			 {0,5,0,1,4},{0,1,10,255,0},{0,5,0,2,4},
			 {0,0,231,255,0},{1,1,0,255,0},{0,5,0,255,4},{0,4,0,255,0},{1,1,0,255,0},{2,1,0,255,0},
			 {0,1,118,255,0},{0,5,0,255,4},{0,6,0,255,0},{0,1,90,255,0},{0,5,0,3,4},{1,0,0,255,0},
			 {0,0,0,0,0},};//初启5左
unsigned char zz16[][5] = {
	             {2,1,0,255,0},{0,4,0,255,0},{0,5,0,102,0},{2,1,0,255,0},
		       {0,5,0,101,0},{0,3,0,255,0},{0,5,0,102,0},{4,1,0,255,0},
			 {0,5,0,101,0},{0,4,0,255,0},{3,1,0,255,0},{0,5,0,103,0},{0,1,3,255,0},
			 {0,4,0,255,0},{0,1,6,255,0},
			 {0,0,0,0,0},};//对抗初启6左
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
unsigned char ss11[][5] = {
			 {2,1,0,255,0},{0,5,0,255,2},{0,3,0,255,0},
			 {3,1,0,255,0},{0,5,0,255,2},{0,4,0,255,0},{2,1,0,255,0},
			 {0,5,0,1,4},{0,1,10,255,0},{0,5,0,2,4},
			 {0,0,231,255,0},{1,1,0,255,0},{0,5,0,255,4},{0,4,0,255,0},{1,1,0,255,0},{3,1,0,103,0},
			 {0,5,0,3,4},{0,1,20,255,0},{0,0,10,255,0},
			 {0,0,0,0,0},};//初启1上
unsigned char ss12[][5] = {
			 {2,1,0,255,0},{0,5,0,255,2},{0,3,0,255,0},
			 {3,1,0,255,0},{0,5,0,255,2},{0,4,0,255,0},{2,1,0,255,0},
			 {0,5,0,1,4},{0,1,10,255,0},{0,5,0,2,4},
			 {0,0,231,255,0},{1,1,0,255,0},{0,5,0,255,4},{0,4,0,255,0},{1,1,0,255,0},{2,1,0,255,0},
			 {0,1,118,255,0},{0,5,0,255,4},{0,7,0,255,0},{0,1,90,255,0},{0,5,0,6,4},{1,0,0,255,0},
			 {0,0,0,0,0},};//初启2上
unsigned char ss13[][5] = {
			 {0,0,0,0,0},};//对抗，右边出发，前推右近
unsigned char ss14[][5] = {
			 {0,0,0,0,0},};//对抗，右边出发，前推右近
unsigned char ss15[][5] = {
			 {0,0,0,0,0},};//对抗，右边出发，前推右近
unsigned char ss16[][5] = {

			 {0,0,0,0,0},};//对抗，右边出发，前推右近
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
unsigned char xx11[][5] = {
			 {2,1,0,255,0},{0,5,0,255,2},{0,4,0,255,0},
			 {1,1,0,255,0},{0,5,0,255,2},{0,3,0,255,0},{2,1,0,255,0},
			 {0,5,0,1,4},{0,1,10,255,0},{0,5,0,2,4},
			 {0,0,231,255,0},{1,1,0,255,0},{0,5,0,255,4},{0,4,0,255,0},{1,1,0,255,0},{3,1,0,103,0},
...
unsigned char xx12[][5] = {
			 ...
			 {0,1,118,255,0},{0,5,0,255,4},{0,7,0,255,0},{0,1,90,255,0},{0,5,0,6,4},{1,0,0,255,0},
			 {0,0,0,0,0},};//初启2下
unsigned char xx13[][5] = {
			 {0,0,0,0,0},};//对抗，右边出发，前推右近
unsigned char xx14[][5] = {
			 {0,0,0,0,0},};//对抗，右边出发，前推右近
unsigned char xx15[][5] = {
			 {0,0,0,0,0},};//对抗，右边出发，前推右近
unsigned char xx16[][5] = {			 
			 {2,1,0,255,0},{0,4,0,255,0},{0,5,0,104,2},{2,1,0,255,0},{0,3,0,255,0},{1,1,0,255,0},{2,1,0,255,0},
			 {0,5,0,101,0},{1,0,0,255,0},{1,4,0,255,0},{2,1,0,255,0},{0,5,0,103,0},{0,1,20,255,0},{0,0,8,255,0},
			 {0,0,0,0,0},};//初启6下
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//五个一组分别表示格数，方向，延时停车的延时时长ms，指令数组下标，等待机械臂延时s
unsigned char yy21[][5] = {
			 {4,1,0,255,0},{0,5,0,255,4},{0,3,0,255,0},{0,1,90,255,0},
			 {0,4,0,255,0},{0,1,4,255,0},{0,5,0,255,4},{0,3,0,255,0},{0,5,0,109,0},                   
			 {0,1,25,255,0},{0,5,0,101,0},{0,0,5,255,0},{0,5,0,255,4},{0,3,0,255,0},{0,0,65,255,0},{0,1,18,255,0},{1,3,0,255,0},
			 {5,1,0,255,0},{0,5,0,103,0},{0,1,20,255,0},{0,0,10,255,0},
			 {0,0,0,0,0},};//重启1右
unsigned char yy22[][5] = {
	             {4,1,0,255,0},{0,5,0,255,4},{1,4,0,255,0},{4,1,0,255,0},{0,5,0,255,4},{0,4,0,255,0},
			 {0,5,0,4,3},{0,1,10,255,0},{0,5,0,5,4},			
			 {0,0,6,255,0},{0,3,0,255,0},{0,5,0,3,4},{1,0,0,255,0},
			 {0,0,0,0,0},};//重启2右
unsigned char yy23[][5] = {
	             {6,1,0,255,0},{0,5,0,255,4},{1,4,0,255,0},{4,1,0,255,0},{0,5,0,255,4},{0,4,0,255,0},
			 {0,5,0,1,3},{0,1,10,255,0},{0,5,0,2,4},			
			 {0,0,6,255,0},{0,3,0,255,0},{0,5,0,3,4},{1,0,0,255,0},
			 {0,0,0,0,0},};//重启3右
unsigned char yy24[][5] = {
	             {6,1,0,255,0},{0,5,0,255,4},{1,4,0,255,0},{4,1,0,255,0},{0,5,0,255,4},{0,3,0,255,0},
			 {0,5,0,4,3},{0,1,10,255,0},{0,5,0,5,4},			
			 {0,0,6,255,0},{0,4,0,255,0},{0,5,0,3,4},{1,0,0,255,0},
			 {0,0,0,0,0},};//重启4右
unsigned char yy25[][5] = {
			 {0,0,0,0,0},};//重启5右
unsigned char yy26[][5] = {
			 {0,0,0,0,0},};//重启6右

//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
unsigned char oo21[][5] = {
                   {0,0,0,0,0},};//对抗初启1中
unsigned char oo22[][5] = {		
			 {2,1,0,255,0},{2,4,0,255,0},{0,3,0,255,10},{0,1,131,255,0},{3,0,0,255,0},
			 {0,0,0,0,0},};//重启2中		中间撞
unsigned char oo23[][5] = {
			 {4,1,0,255,0},{0,5,0,255,2},{0,4,0,103,0},{1,1,0,255,0},{4,1,0,255,0},{0,1,20,255,0},{0,0,10,255,0},
			 {0,0,0,0,0},};//重启3中	
unsigned char oo24[][5] = {
			 {6,1,0,255,0},{0,5,0,255,2},{0,4,0,103,0},{1,1,0,255,0},{4,1,0,255,0},{0,1,20,255,0},{0,0,10,255,0},
			 {0,0,0,0,0},};//重启4中	
unsigned char oo25[][5] = {
			 {4,1,0,255,0},{0,5,0,255,4},{0,3,0,255,0},{0,1,70,255,0},{0,5,0,103,0},
			 {0,1,30,255,0},{0,5,0,101,0},{0,0,8,255,0},
			 {0,5,0,255,4},{0,4,0,255,0},{1,4,0,255,0},			 
			 {5,1,0,255,0},{0,5,0,103,0},{0,1,20,255,0},{0,0,10,255,0},
			 {0,0,0,0,0}}; //重启5中	
unsigned char oo26[][5] = {
			 {6,1,0,255,0},{0,5,0,255,4},{0,3,0,255,0},{0,1,70,255,0},{0,5,0,103,0},
			 {0,1,30,255,0},{0,5,0,101,0},{0,0,8,255,0},
			 {0,5,0,255,4},{0,4,0,255,0},{1,4,0,255,0},			 
			 {5,1,0,255,0},{0,5,0,103,0},{0,1,20,255,0},{0,0,10,255,0},
			 {0,0,0,0,0},};//重启6中	
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
unsigned char zz21[][5] = {
			  {4,1,0,255,0},{0,5,0,255,4},{0,3,0,255,0},{0,1,90,255,0},
			  {0,3,0,255,0},{0,1,10,255,0},{0,5,0,255,4},{0,4,0,255,0},{0,5,0,109,0},                   
			  {0,1,25,255,0},{0,5,0,101,0},{0,0,5,255,0},{0,5,0,255,4},{0,4,0,255,0},{0,0,65,255,0},{0,1,18,255,0},{1,4,0,255,0},
			  {5,1,0,255,0},{0,5,0,103,0},{0,1,20,255,0},{0,0,10,255,0},
                    {0,0,0,0,0},};//重启1左
unsigned char zz22[][5] = {
	             {2,1,0,255,0},{0,5,0,255,4},{0,3,0,255,0},{1,1,0,255,0},{0,4,0,255,0},
			 {0,5,0,4,3},{0,1,10,255,0},{0,5,0,5,4},
			 {0,0,231,255,0},{1,1,0,255,0},{0,5,0,255,4},{0,4,0,255,0},{1,1,0,255,0},{5,1,0,255,0},
			 {0,1,112,255,0},{0,5,0,255,4},{0,6,0,255,0},{0,1,91,255,0},{0,5,0,6,4},
			 {0,0,0,0,0},};//重启2左
unsigned char zz23[][5] = {
	             {4,1,0,255,0},{0,5,0,255,4},{0,3,0,255,0},{1,1,0,255,0},{0,4,0,255,0},
			 {0,5,0,1,3},{0,1,10,255,0},{0,5,0,2,4},
			 {0,0,6,255,0},{0,5,0,255,4},{0,4,0,255,0},{5,1,0,255,0},
			 {0,1,118,255,0},{0,5,0,255,4},{0,6,0,255,0},{0,1,90,255,0},{0,5,0,3,4},
			 {0,0,0,0,0},};//重启3左
unsigned char zz24[][5] = {
	             {2,1,0,255,0},{0,5,0,255,4},{0,4,0,255,0},{2,1,0,255,0},{0,5,0,255,2},{0,3,0,255,0},
                   {4,1,0,255,0},{0,5,0,255,2},{0,3,0,255,0},{3,1,0,255,0},{0,5,0,255,2},{0,4,0,255,0},
			 {0,5,0,4,3},{0,1,10,255,0},{0,5,0,5,4},
			 {0,0,7,255,0},{0,5,0,255,4},{0,4,0,255,0},{1,1,0,255,0},{4,1,0,255,0},
			 {0,1,110,255,0},{0,5,0,255,4},{0,6,0,255,0},{0,1,90,255,0},{0,5,0,6,4},
			 {0,0,0,0,0},};//重启4左
unsigned char zz25[][5] = {
			 {8,1,0,255,0},{0,5,0,255,2},{0,3,0,255,0},
			 {1,1,0,255,0},{0,1,12,255,0},{0,5,0,255,2},{0,4,0,109,0},
			 {1,1,0,255,0},{0,1,9,255,0},{0,5,0,101,0},{0,0,45,255,0},
			 {0,5,0,255,4},{0,4,0,255,0},{5,1,0,255,0},{0,4,0,255,0},{2,1,0,255,0},
			 {0,3,0,255,0},{2,1,0,255,0},
			 {0,5,0,103,0},{0,1,20,255,0},{0,0,10,255,0},
			 {0,0,0,0,0},};//重启5左
unsigned char zz26[][5] = {
	             {4,1,0,255,0},{0,5,0,255,4},{0,3,0,255,0},{1,1,0,255,0},{0,4,0,255,0},
			 {0,5,0,1,3},{0,1,10,255,0},{0,5,0,2,4},
			 {0,0,6,255,0},{0,5,0,255,4},{0,4,0,255,0},{5,1,0,255,0},
			 {0,1,118,255,0},{0,5,0,255,4},{0,7,0,255,0},{0,1,90,255,0},{0,5,0,6,4},
			 {0,0,0,0,0},};//重启6左
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
unsigned char ss21[][5] = {
			 {6,1,0,255,0},{0,5,0,255,4},{0,3,0,255,0},{0,1,90,255,0},
			 {0,4,0,255,0},{0,1,4,255,0},{0,5,0,255,4},{0,3,0,255,0},{0,5,0,109,0},                   
			 {0,1,25,255,0},{0,5,0,101,0},{0,0,5,255,0},{0,5,0,255,4},{0,3,0,255,0},{0,0,65,255,0},{0,1,18,255,0},{1,3,0,255,0},
			  {5,1,0,255,0},{0,5,0,103,0},{0,1,20,255,0},{0,0,10,255,0},
			 {0,0,0,0,0},};//重启1上
unsigned char ss22[][5] = {
	             {5,1,0,255,0},{0,5,0,255,4},{0,4,0,255,0},
			 {0,5,0,4,3},{0,1,10,255,0},{0,5,0,5,4},
			 {0,0,7,255,0},{0,5,0,255,4},{0,4,0,255,0},{1,1,0,255,0},{4,1,0,255,0},
			 {0,1,110,255,0},{0,5,0,255,4},{0,6,0,255,0},{0,1,90,255,0},{0,5,0,6,4},
			 {0,0,0,0,0},};//重启2上
unsigned char ss23[][5] = {
	             {4,1,0,255,0},{0,5,0,255,4},{0,3,0,255,0},{1,1,0,255,0},{0,4,0,255,0},
			 {0,5,0,1,3},{0,1,10,255,0},{0,5,0,2,4},
			 {0,0,6,255,0},{0,5,0,255,4},{0,4,0,255,0},{5,1,0,255,0},
			 {0,1,118,255,0},{0,5,0,255,4},{0,6,0,255,0},{0,1,90,255,0},{0,5,0,3,4},
			 {0,0,0,0,0},};//重启3上
unsigned char ss24[][5] = {
			 {0,0,0,0,0},};//重启4上
unsigned char ss25[][5] = {
			 {2,1,0,255,0},{0,5,0,255,2},{0,4,0,255,0},
			 {2,1,0,255,0},{0,5,0,255,2},{0,3,0,255,0},
			 {6,1,0,255,0},{0,5,0,255,2},{0,4,0,255,0},
			 {3,1,0,255,0},{0,1,12,255,0},{0,3,0,109,0},
			 {1,1,0,255,0},{0,1,9,255,0},{0,5,0,101,0},{0,0,45,255,0},
			 {0,5,0,255,4},{0,3,0,255,0},{3,1,0,255,0},{0,5,0,255,2},{0,3,0,255,0},
			 {2,1,0,255,0},{0,3,0,255,0},{2,1,0,255,0},
			 {0,5,0,103,0},{0,1,20,255,0},{0,0,10,255,0},
			 {0,0,0,0,0},};//重启5上
unsigned char ss26[][5] = {
			 {6,1,0,255,0},{0,5,0,255,4},{0,3,0,255,0},{1,1,0,255,0},{0,4,0,255,0},
			 {0,5,0,4,3},{0,1,10,255,0},{0,5,0,5,4},
			 {0,0,6,255,0},{0,5,0,255,4},{0,4,0,255,0},{1,1,0,255,0},{4,1,0,255,0},
			 {0,1,112,255,0},{0,5,0,255,4},{0,6,0,255,0},{0,1,91,255,0},{0,5,0,6,4},
			 {0,0,0,0,0},};//重启6上
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
unsigned char xx21[][5] = {
			  {6,1,0,255,0},{0,5,0,255,4},{0,3,0,255,0},{0,1,90,255,0},
			  {0,3,0,255,0},{0,1,10,255,0},{0,5,0,255,4},{0,4,0,255,0},{0,5,0,109,0},                   
			  {0,1,25,255,0},{0,5,0,101,0},{0,0,5,255,0},{0,5,0,255,4},{0,4,0,255,0},{0,0,65,255,0},{0,1,18,255,0},{1,4,0,255,0},
			  {5,1,0,255,0},{0,5,0,103,0},{0,1,20,255,0},{0,0,10,255,0},
                    {0,0,0,0,0},};//重启1下
unsigned char xx22[][5] = {
			 {0,0,0,0,0},};//重启2下
unsigned char xx23[][5] = {
			 {0,0,0,0,0},};//重启3下
unsigned char xx24[][5] = {
			 {0,0,0,0,0},};//重启4下
unsigned char xx25[][5] = {
			 {2,1,0,255,0},{0,5,0,255,4},{0,3,0,255,0},
			 {1,1,0,255,0},{0,1,12,255,0},{0,3,0,109,0},
			 {1,1,0,255,0},{0,1,12,255,0},{0,5,0,101,6},{0,0,45,255,0},	{0,5,0,255,2},
			 {0,3,0,255,0},{5,1,0,255,0},{0,5,0,255,4},{0,3,0,255,0},{2,1,0,255,0},{0,5,0,255,2},
			 {0,4,0,255,0},{2,1,0,255,0},{0,5,0,103,0},{0,1,20,255,0},{0,0,10,255,0},
			 {0,0,0,0,0},};//重启5下
unsigned char xx26[][5] = {
			 
                   {0,0,0,0,0},};//重启6下
//五个一组分别表示格数，方向，延时停车的延时时长ms，指令数组下标，等待机械臂延时s
unsigned char yy31[][5] = {
	             {2,1,0,255,0},{0,5,0,255,4},{0,3,0,255,0},{2,1,0,255,0},{0,4,0,255,0},
			 {2,1,0,255,0},{0,5,0,255,4},{0,3,0,255,0},{2,1,0,255,0},{0,1,90,255,0},
			 {0,4,0,255,0},{0,1,4,255,0},{0,5,0,255,4},{0,3,0,255,0},{0,5,0,109,0},                   
			 {0,1,25,255,0},{0,5,0,101,0},{0,0,5,255,0},{0,5,0,255,4},{0,3,0,255,0},{0,0,65,255,0},{0,1,18,255,0},{1,3,0,255,0},
			 {5,1,0,255,0},{0,5,0,103,0},{0,1,20,255,0},{0,0,10,255,0},
			 {0,0,0,0,0},};//再启1右
unsigned char yy32[][5] = {
	             {2,1,0,255,0},{0,5,0,255,4},{0,3,0,255,0},{2,1,0,255,0},{0,4,0,255,0},
			 {4,1,0,255,0},{0,5,0,255,2},{0,3,0,255,0},{3,1,0,255,0},{0,5,0,255,4},{0,4,0,255,0},
			 {0,5,0,4,3},{0,1,8,255,0},{0,5,0,5,4},			
			 {0,0,6,255,0},{0,5,0,255,2},{0,4,0,255,0},
			 {1,1,0,255,0},{4,1,0,255,0},
			 {0,1,110,255,0},{0,5,0,255,4},{0,6,0,255,0},{0,1,90,255,0},{0,5,0,6,4},
                   {0,0,0,0,0},};;//再启2右
unsigned char yy33[][5] = {
	             {2,1,0,255,0},{0,5,0,255,4},{0,3,0,255,0},{2,1,0,255,0},{0,4,0,255,0},
			 {2,1,0,255,0},{0,5,0,255,2},{0,3,0,255,0},{3,1,0,255,0},{0,5,0,255,4},{0,4,0,255,0},
			 {0,5,0,1,3},{0,1,10,255,0},{0,5,0,2,6},			
			 {0,0,6,255,0},{0,5,0,255,2},{0,4,0,255,0},{1,1,0,255,0},{4,1,0,255,0},
			 {0,1,117,255,0},{0,5,0,255,4},{0,6,0,255,0},{0,1,92,255,0},{0,5,0,3,4},	 
			 {0,0,0,0,0},};//再启3右
unsigned char yy34[][5] = {
			 {2,1,0,255,0},{0,5,0,255,2},{0,3,0,255,0},
			 {3,1,0,255,0},{0,5,0,255,2},{0,4,0,255,0},{2,1,0,255,0},
			 {0,5,0,1,4},{0,1,10,255,0},{0,5,0,2,4},
			 {0,0,9,255,0},{0,5,0,255,5},{0,4,0,255,0},{3,1,0,255,0},
			 {0,1,118,255,0},{0,5,0,255,4},{0,6,0,255,0},{0,1,90,255,0},{0,5,0,3,4},{1,0,0,255,0},
			 {0,0,0,0,0},};//再启4右	
unsigned char yy35[][5] = {
	             {2,1,0,255,0},{0,5,0,255,4},{0,4,0,255,0},
			 {1,1,0,255,0},{0,1,12,255,0},{0,5,0,255,2},{0,4,0,109,0},
			 {1,1,0,255,0},{0,1,12,255,0},{0,5,0,101,6},{0,0,45,255,0},	
                   {0,5,0,255,2},{0,4,0,255,0},{3,1,0,255,0},{0,4,0,255,0},
			 {2,1,0,255,0},{0,4,0,255,0},{2,1,0,255,0},{0,5,0,103,0},{0,1,20,255,0},{0,0,10,255,0},
			 {0,0,0,0,0},};//再启5右
unsigned char yy36[][5] = {
	             {2,1,0,255,0},{0,5,0,255,4},{0,3,0,255,0},{2,1,0,255,0},{0,4,0,255,0},
			 {1,1,0,255,0},{5,1,0,255,0},{0,5,0,255,2},{0,3,0,255,0},{1,1,0,255,0},{2,1,0,255,0},{0,5,0,255,4},{0,3,0,255,0},
			 {0,5,0,4,3},{0,1,8,255,0},{0,5,0,5,4},			
			 {0,0,231,255,0},{1,1,0,255,0},{0,5,0,255,2},{0,3,0,255,0},
			 {1,1,0,255,0},{4,1,0,255,0},
			 {0,1,110,255,0},{0,5,0,255,4},{0,7,0,255,0},{0,1,90,255,0},{0,5,0,6,4},
			 {0,0,0,0,0},};//再启6右


//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
unsigned char oo31[][5] = {
	             {2,1,0,255,0},{0,5,0,255,4},{0,3,0,255,0},{2,1,0,255,0},{0,4,0,255,0},
			 {2,1,0,255,0},{0,5,0,255,2},{0,3,0,255,0},{2,1,0,255,0},{0,1,70,255,0},{0,5,0,103,0},
			 {0,1,30,255,0},{0,5,0,101,0},{0,0,8,255,0},
			 {0,5,0,255,4},{0,4,0,255,0},{1,4,0,255,0},			 
			 {5,1,0,255,0},{0,5,0,103,0},{0,1,20,255,0},{0,0,10,255,0},
			 {0,0,0,0,0},};//再启1中
unsigned char oo32[][5] = {		
	             {2,1,0,255,0},{0,5,0,255,4},{0,3,0,255,0},{2,1,0,255,0},{0,4,0,255,0},
			 {4,1,0,255,0},{0,5,0,255,4},{0,3,0,255,0},{2,1,0,255,0},{0,1,70,255,0},{0,5,0,103,0},
			 {0,1,30,255,0},{0,5,0,101,0},{0,0,8,255,0},
			 {0,5,0,255,4},{0,4,0,255,0},{1,4,0,255,0},			
		       {5,1,0,255,0},{0,5,0,103,0},{0,1,20,255,0},{0,0,10,255,0},
			 {0,0,0,0,0},};//再启2中
unsigned char oo33[][5] = {
	             {2,1,0,255,0},{0,5,0,255,4},{0,3,0,255,0},{1,1,0,255,0},{2,1,0,255,0},{0,4,0,255,0},
			 {2,1,0,255,0},{0,5,0,255,2},{0,3,0,255,0},{2,1,0,255,0},{0,5,0,255,4},{0,4,0,255,0},
			 {0,5,0,1,3},{0,1,10,255,0},{0,5,0,2,4},			
			 {0,0,6,255,0},{0,5,0,255,2},{0,4,0,255,0},{1,1,0,255,0},{4,1,0,255,0},
			 {0,1,117,255,0},{0,5,0,255,4},{0,7,0,255,0},{0,1,92,255,0},{0,5,0,6,4},
			 {0,0,0,0,0},};//再启3中
unsigned char oo34[][5] = {
			 {2,1,0,255,0},{0,5,0,255,4},{0,4,0,255,0},{1,1,0,255,0},
			 {0,5,0,255,4},{0,3,0,255,0},{0,5,0,4,4},{0,1,10,255,0},{0,5,0,5,4},{0,0,6,255,0},
			 {0,4,0,255,0},{0,1,85,255,0},{0,5,0,3,4},{1,0,0,255,0},
			 {0,0,0,0,0},};//再启4中
unsigned char oo35[][5] = {
			 {2,1,0,255,0},{0,5,0,255,4},{0,3,0,255,0},{1,1,0,255,0},
			 {0,5,0,255,4},{2,4,0,255,0},{0,5,0,255,4},{2,4,0,255,0},
			 {0,5,0,255,4},{0,3,0,255,0},{0,5,0,1,6},{0,1,10,255,0},{0,5,0,2,4},{0,0,6,255,0},
			 {0,4,0,255,0},{0,5,0,3,4},{1,0,0,255,0},
			 {0,0,0,0,0}}; //再启5中
unsigned char oo36[][5] = {
			 {2,1,0,255,0},{0,5,0,255,4},{0,3,0,255,0},{2,1,0,255,0},
			 {0,5,0,255,4},{1,4,0,255,0},{3,1,0,255,0},{0,5,0,255,4},{3,4,0,255,0},
			 {0,5,0,255,4},{0,3,0,255,0},{0,5,0,4,6},{0,1,10,255,0},{0,5,0,5,4},{0,0,6,255,0},
			 {0,4,0,255,0},{0,5,0,3,4},{1,0,0,255,0},
			 {0,0,0,0,0},};//再启6中
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
unsigned char zz31[][5] = {
	              {2,1,0,255,0},{0,5,0,255,4},{0,3,0,255,0},{2,1,0,255,0},{0,4,0,255,0},
			  {2,1,0,255,0},{0,5,0,255,2},{0,3,0,255,0},{2,1,0,255,0},{0,1,90,255,0},
			  {0,3,0,255,0},{0,1,4,255,0},{0,5,0,255,4},{0,4,0,255,0},{0,5,0,109,0},                   
			  {0,1,25,255,0},{0,5,0,101,0},{0,0,5,255,0},{0,5,0,255,4},{0,4,0,255,0},{0,0,65,255,0},{0,1,18,255,0},{1,4,0,255,0},
			  {5,1,0,255,0},{0,5,0,103,0},{0,1,20,255,0},{0,0,10,255,0},
			  {0,0,0,0,0},};//再启1左
unsigned char zz32[][5] = {
	             {2,1,0,255,0},{0,5,0,255,2},{0,3,0,255,0},{2,1,0,255,0},{0,4,0,255,0},
			 {2,1,0,255,0},{0,5,0,255,2},{0,3,0,255,0},{3,1,0,255,0},{0,5,0,255,2},{0,3,0,255,0},				 
                   {0,5,0,4,3},{0,1,10,255,0},{0,5,0,5,4},
			 {0,0,6,255,0},{0,5,0,255,4},{0,3,0,255,0},{5,1,0,255,0},
		     	 {0,1,115,255,0},{0,5,0,255,4},{0,6,0,255,0},{0,1,90,255,0},{0,5,0,3,4},
			 {0,0,0,0,0},};//再启2左
unsigned char zz33[][5] = {
	         {2,1,0,255,0},{0,5,0,255,4},{0,3,0,255,0},{2,1,0,255,0},{0,4,0,255,0},
			 {4,1,0,255,0},{0,5,0,255,2},{0,3,0,255,0},{3,1,0,255,0},{0,5,0,255,2},{0,3,0,255,0},
			 {0,5,0,1,3},{0,1,10,255,0},{0,5,0,2,4},			
			 {0,0,6,255,0},{0,5,0,255,4},{0,3,0,255,0},{5,1,0,255,0},
			 {0,1,114,255,0},{0,5,0,255,4},{0,7,0,255,0},{0,1,90,255,0},{0,5,0,3,4},
			 {0,0,0,0,0},};//再启3左
unsigned char zz34[][5] = {
			 {2,1,0,255,0},{0,5,0,255,4},{0,3,0,255,0},{2,1,0,255,0},{0,5,0,255,2},{0,4,0,255,0},
			 {2,1,0,255,0},{0,4,0,103,0},{1,1,0,255,0},{2,1,0,255,0},
			 {0,1,20,255,0},{0,0,10,255,0},
			 {0,0,0,0,0},};//再启4左
unsigned char zz35[][5] = {
	             {2,1,0,255,0},{0,5,0,255,2},{0,3,0,255,0},
			 {2,1,0,255,0},{0,5,0,255,2},{0,4,0,255,0},
			 {6,1,0,255,0},{0,5,0,255,2},{0,3,0,255,0},
			 {3,1,0,255,0},{0,1,12,255,0},{0,5,0,255,4},{0,4,0,109,0},
			 {1,1,0,255,0},{0,1,9,255,0},{0,5,0,101,0},
			 {0,0,45,255,0},{0,5,0,255,4},{0,4,0,255,0},
                   {4,1,0,255,0},{0,4,0,255,0},{2,1,0,255,0},
			 {0,3,0,255,0},{3,1,0,255,0},{0,5,0,103,0},{0,1,20,255,0},{0,0,10,255,0},
			 {0,0,0,0,0},};//再启5左
unsigned char zz36[][5] = {
			 {2,1,0,255,0},{0,5,0,255,4},{1,3,0,255,0},{3,1,0,255,0},{0,5,0,255,2},{0,4,0,255,0},
			 {2,1,0,255,0},{0,4,0,103,0},{1,1,0,255,0},{2,1,0,255,0},
			 {0,1,20,255,0},{0,0,10,255,0},
			 {0,0,0,0,0},};//再启6左
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
unsigned char ss31[][5] = {
	             {2,1,0,255,0},{0,5,0,255,4},{0,3,0,255,0},{2,1,0,255,0},{0,4,0,255,0},
			 {4,1,0,255,0},{0,5,0,255,4},{0,3,0,255,0},{2,1,0,255,0},{0,1,90,255,0},
			 {0,4,0,255,0},{0,1,4,255,0},{0,5,0,255,2},{0,3,0,255,0},{0,5,0,109,0},                   
			 {0,1,25,255,0},{0,5,0,101,0},{0,0,5,255,0},{0,5,0,255,4},{0,3,0,255,0},{0,0,65,255,0},{0,1,18,255,0},{1,3,0,255,0},
			 {5,1,0,255,0},{0,5,0,103,0},{0,1,20,255,0},{0,0,10,255,0},
			 {0,0,0,0,0},};//再启1上
unsigned char ss32[][5] = {
			 {2,1,0,255,0},{0,5,0,255,2},{1,3,0,255,0},{0,5,0,255,2},{2,4,0,255,0},
			 {0,5,0,255,2},{1,4,0,255,0},{0,8,0,255,0},{0,5,0,1,4},	
			 {0,1,30,255,0},{0,0,10,255,0},
			 {0,0,0,0,0},};//再启2上
unsigned char ss33[][5] = {
			 {2,1,0,255,0},{0,5,0,255,2},{0,9,0,255,0},{0,5,0,4,4},	
			 {0,1,30,255,0},{0,0,10,255,0},
			 {0,0,0,0,0},};//再启3上
unsigned char ss34[][5] = {
			 {2,1,0,255,0},{0,5,0,255,2},{0,4,0,255,0},{1,1,0,255,0},{0,1,10,255,0},{0,5,0,255,2},{0,3,0,255,0},{0,5,0,4,4},
			 {0,1,15,255,0},{0,0,10,255,0},			 
			 {0,0,0,0,0},};//再启4上
unsigned char ss35[][5] = {
	             {2,1,0,255,0},{0,5,0,255,2},{0,3,0,255,0},
			 {2,1,0,255,0},{0,5,0,255,2},{0,4,0,255,0},
			 {6,1,0,255,0},{0,5,0,255,2},{0,4,0,255,0},
			 {3,1,0,255,0},{0,1,12,255,0},{0,3,0,109,0},
			 {1,1,0,255,0},{0,1,9,255,0},{0,5,0,101,0},{0,0,45,255,0},
			 {0,5,0,255,4},{0,3,0,255,0},{3,1,0,255,0},{0,5,0,255,2},{0,3,0,255,0},
			 {2,1,0,255,0},{0,5,0,255,2},{0,3,0,255,0},{2,1,0,255,0},
			 {0,5,0,103,0},{0,1,20,255,0},{0,0,10,255,0},
			 {0,0,0,0,0},};//再启5上
unsigned char ss36[][5] = {
			 {2,1,0,255,0},{0,5,0,255,2},{1,3,0,255,0},{0,5,0,255,2},{2,4,0,255,0},
			 {0,5,0,255,2},{2,4,0,255,0},
			 {0,1,10,255,0},{0,5,0,255,2},{0,3,0,255,0},{0,5,0,1,4},
			 {0,1,15,255,0},{0,0,10,255,0},
			 {0,0,0,0,0},};//再启6上
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
unsigned char xx31[][5] = {
	              {2,1,0,255,0},{0,5,0,255,4},{0,3,0,255,0},{2,1,0,255,0},{0,4,0,255,0},
			  {4,1,0,255,0},{0,5,0,255,2},{0,3,0,255,0},{2,1,0,255,0},{0,1,90,255,0},
			  {0,3,0,255,0},{0,1,4,255,0},{0,5,0,255,4},{0,4,0,255,0},{0,5,0,109,0},                   
			  {0,1,25,255,0},{0,5,0,101,0},{0,0,5,255,0},{0,5,0,255,4},{0,4,0,255,0},{0,0,65,255,0},{0,1,18,255,0},{1,4,0,255,0},
			  {5,1,0,255,0},{0,5,0,103,0},{0,1,20,255,0},{0,0,10,255,0},
			  {0,0,0,0,0},};//再启1下
unsigned char xx32[][5] = {
			 {0,0,0,0,0},};//再启2下
unsigned char xx33[][5] = {
			 {0,0,0,0,0},};//再启3下
unsigned char xx34[][5] = {
			 {0,0,0,0,0},};//再启4下
unsigned char xx35[][5] = { 
			 {2,1,0,255,0},{0,5,0,255,4},{2,3,0,255,0},{0,5,0,255,4},{4,4,0,255,0},
			 {0,5,0,255,2},{3,4,0,255,0},{0,1,5,255,0},{0,5,0,255,2},{0,4,0,255,0},
			 {0,5,0,1,4},
			 {0,1,15,255,0},{0,0,10,255,0},
			 {0,0,0,0,0},};//再启5下
unsigned char xx36[][5] = {
			 {2,1,0,255,0},{0,5,0,255,4},{2,3,0,255,0},{0,5,0,255,4},{6,4,0,255,0},
			 {0,5,0,255,2},{3,4,0,255,0},{0,1,5,255,0},{0,5,0,255,2},{0,4,0,255,0},
			 {0,5,0,4,4},
			 {0,1,15,255,0},{0,0,10,255,0}, 
			 {0,0,0,0,0},};//再启6下
...
StatusTab KeyTab[MENU_SIZE] =
	{
		...
		{17, 20, 19, 18, 17, 17, (*Clock_b)}, //自设1
		{18, 20, 17, 19, 18, 18, (*Clock_c)}, //自设2
		{19, 20, 18, 17, 19, 19, (*Clock_d)}, //自设3
		{20, 20, 20, 20, 16, 20, (*Clock_e)}, //路线观看
		...
		angle_cs=angle_6;
				if(i==0)
				{
					if(a[i][1]>=200)	//此时为双车
					{
						...
						printf("%c\r\n",'9');
						USART_PRINTF_FLAG = 2;
						while(x_c!='1'); //x_c=1的时候 大车已经开出起始点，小车下车。
						printf("%s", zhiling[0]);
						USART_PRINTF_FLAG = 3;
						flag_shuang=1;
						a[i][1]=0;
					...
					if(a[i][2]!=0&&flag_shuang==1)
					{
						if(a[i][2]>230)
								DJ_MOVE_YS_XC(a[i][0]-1,a[i][1],1,a[i][2]-180);//延时停车函数
						else if(a[i][2]>160)
								DJ_MOVE_YS_XC(a[i][0]-1,a[i][1],6,a[i][2]-20);//延时停车函数
						else 
								DJ_MOVE_YS_XC(a[i][0]-1,a[i][1],5,a[i][2]);//延时停车函数
					}
					else if(a[i][2]!=0&&flag_shuang==0)
					{
						if(a[i][2]>230)
								DJ_MOVE_YS(a[i][0]-1,a[i][1],1,a[i][2]-180);//延时停车函数
						else if(a[i][2]>160)
								DJ_MOVE_YS(a[i][0]-1,a[i][1],6,a[i][2]-20);//延时停车函数
						else if(a[i][2]>50)
								DJ_MOVE_YS(a[i][0]-1,a[i][1],5,a[i][2]);//延时停车函数
						else
								BMQ_MOVE(a[i][1],a[i][2],0);//编码器函移动函数
					}
					else if(a[i][1]!=2)  
					{
								BMQ_MOVE(1,40,0);//出门第一格用编码器	40；
								DJ_MOVE_BUFF_SET(a[i][0]-1,a[i][1]);
					}
					if(a[i][3]!=255)
					{
										delay_ms(100);
										printf("%s", zhiling[a[i][3]]); //发送舵机串口指令
					}
						for(j=0;j<a[i][4];j++)
						delay_ms(100);
						...
						USART_PRINTF_FLAG = 2; //串口不再用于蓝牙而用于舵机
				...
				
				{
					if (a[i][2] != 0)
						{	
							...
							DJ_MOVE_YS(a[i][0],a[i][1],1,a[i][2]-180);//延时停车函数
							else if(a[i][2]>140)
							DJ_MOVE_YS(a[i][0],a[i][1],6,a[i][2]-20);//延时停车函数
							else if(a[i-1][1]==3||a[i-1][1]==4||a[i-1][1]==1||a[i-1][1]==0)
							DJ_MOVE_YS(a[i][0], a[i][1], 5, a[i][2]);
							else
							BMQ_MOVE(a[i][1], a[i][2], 0);
						}

					else if (a[i][1] != 2)
						DJ_MOVE_BUFF_SET(a[i][0], a[i][1]); //循迹移动函数   格数  方向
